# -*- coding: utf-8 -*-
"""
BİYOEKONOMİ BÜLTENİ — TASLAK PIPELINE (CRON 1 — Pazar 12:00 TSİ)
====================================================================
Akış:
...
def triyaj(adaylar, bas, bit, state):
    ...
    # eşiği 4096 token; bu blok ~1.900-2.500 token (önceki olay listesi
    # doluyken ~3.500). Eşiğin altında kalan istek SESSİZCE cache'lenmez —
    # hata dönmez, yalnızca cache_creation_input_tokens 0 gelir. Yani
    # cache=True burada büyük olasılıkla hiç işe yaramıyor; zarar da vermiyor
    # (yazma ücreti ancak cache oluşursa doğar). Gerçek tasarruf isteniyorsa
    # triyaj modelini eşiği 1024 olan Sonnet 5'e almak gerekir — o da parti
    # başına maliyeti artırır. Bilinçli olarak OLDUĞU GİBİ bırakıldı.
...
ESLESME = {
    ...
    "biyoyakit-enerji": "biyoyakit", "saf": "biyoyakit", "biyogaz": "biyoyakit",
    ...
    "ccu": "karbon", "biyokomur": "karbon", "karbon-yakalama": "karbon",
    "alg": "deniz", "yosun": "deniz", "marine": "deniz",
...
def mock_taslak(sayi_no, bas, bit, pencere):
    def st(i, secim, kat, baslik):
        return {
            ...
            "title": baslik,
            "excerpt": f"Örnek özet {i}: anlaşma 240 milyon dolar değerinde, "
                       f"kapasite 500 bin ton/yıl. Bu bir test metnidir, gerçek haber değildir.",
            "detail": ("Bu bir TEST haberidir; gerçek bir gelişmeyi yansıtmaz.\n\n"
                       "İkinci paragraf: proje kapsamında yıllık 500 bin ton "
                       "kapasiteli bir biyorafineri planlanıyor, toplam yatırım "
                       "240 milyon dolar.\n\n"
                       "Üçüncü paragraf: takvim paylaşılmadı."),
            "neden_onemli": None, "category": kat, "subcategories": [],
            "value_chain": ["donusum-fermantasyon"], "maturity": "announced",
            "companies": ["Örnek A.Ş."], "countries": ["Finland"],
            "technologies": ["HVO"], "capacity": "500 bin ton/yıl",
            "investment": {"amount_original": 240, "currency": "USD",
                           "amount_usd_million": 240,
        ...
        "radar": [
            {"kume": "SAF yatırımları",
             "maddeler": [{"title": f"[TEST] Radar maddesi {i+1}",
                           "source": "Biofuels Digest", "url": f"https://example.org/radar-{i}",
                           "date": bit, "category": "biyoyakit"} for i in range(4)]},
            {"kume": "Biyoplastik tesisleri",
             "maddeler": [{"title": f"[TEST] Radar maddesi {i+5}",
                           "source": "Bioplastics News", "url": f"https://example.org/radar-{i+4}",
                           "date": bit, "category": "biyomalzeme"} for i in range(4)]},
        ],
    }
...
def main():
    ...
    ap.add_argument("--mini", type=int, nargs="?", const=3, metavar="N",
                    help="KÜÇÜK PROVA: ilk N sorgu (vars. 3), 3 derin olay, "
                         ...
                         "sayı üretmez (prompt hâlâ 8-10 haber ister). "
                         "--dry-run ile birlikte kullanın.")
    args = ap.parse_args()
    ...
    # ── KÜÇÜK PROVA ──
    # Sistem değişikliklerinden sonra (model/sağlayıcı geçişi gibi) tüm
    # zinciri gerçek veriyle sınamak gerekir; ama tam çalışma ~15 dk sürüyor
    # ve Exa + model masrafı doğuruyor. --mini ölçeği küçültür, akış aynı kalır.
    # ⚠ Yalnızca komut satırı bayrağı — cron "python pipeline.py" olarak
    # çalıştığı için üretime sızma ihtimali yok.
    if args.mini:
    ...
    if args.mini:
        ...
        if not args.dry_run:
            log("  ⚠ --dry-run YOK: taslak Neon'a yazılacak ve hakemlere "
                "davet gidecek!")
