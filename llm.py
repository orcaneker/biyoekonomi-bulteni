...
def _openrouter_saglayici():
    ...
    Bu yüzden varsayılan: Anthropic'in kendi altyapısına sabitle.

    ⚠ TERS TEPEBİLİR: kurumsal hesabın izin verdiği sağlayıcılar Anthropic'i
    içermiyorsa (ya da bölge/veri politikası filtreleri onu eliyorsa) istek
    404 "No endpoints found for ..." döner — model var, ama filtre sonrası
    elde uç kalmamıştır. Gerçek vaka: 22 Eylül 2026, ilk OpenRouter denemesi.
    Çözüm kod değiştirmeden ortam değişkeniyle:
    ...
    ⚠ Listede "sadece:" yoksa allow_fallbacks BİLEREK True: katı sabitleme
    tam da yukarıdaki 404'e yol açan şeydi. Tercih bildirmek güvenli,
    kilitlemek değil.
    """
...
def _anthropic(model, ad, sistem, kullanici, max_tokens, stream, cache,
    ...
    Gövde AYNI kalır; yalnızca URL, kimlik başlığı ve sağlayıcı sabitleme
    bloğu değişir. Bu yüzden cache, effort ve akış çözümleme kodu ortaktır.
    ...
    if gecit == "openrouter":
        ...
        basliklar = {
            ...
            "X-Title": "Biyoekonomi Bulteni",   # OpenRouter panelinde görünen ad
        }
